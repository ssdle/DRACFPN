...
class MambaVisionMixer(nn.Module):
    def __init__(
        ...
        self.d_inner_one = 256
        self.dt_rank = math.ceil(self.d_model / 16) if dt_rank == "auto" else dt_rank
        self.use_fast_path = use_fast_path
        self.layer_idx = layer_idx
        self.in_proj = nn.Linear(self.d_model, self.d_inner_one, bias=bias, **factory_kwargs)
        ...
        self.mlp = Mlp(in_features=d_model, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=attn_drop)
    ...
    def forward(self, hidden_states):
        ...
        """
        hb,hn,hw,hh = hidden_states.shape
        hidden_states = self.norm1(hidden_states)
        hidden_states = hidden_states.view(hb, hn, -1).transpose(1, 2)
        ...
        y = self.out_proj(y)
        y_norm = self.drop_path(self.gamma_1 * y)
        y_norm = y_norm.transpose(1, 2).reshape(-1, 256, hh, hw)
        y_norm = self.norm2(y_norm)
        y_norm = y_norm.view(hb, hn, -1).transpose(1, 2)
        y = y + self.drop_path(self.gamma_2 * self.mlp(y_norm))
        y = y.transpose(1, 2).reshape(-1, 256, hh, hw)
        return y
